...
class Covid:
    ...
    def adjust_period(self, period="W"):
    ...
    def state_wide(self):
        ...
        '''
        # Groupby County and resample to weekly data
        self.state = self.state.groupby('County').resample('W').sum()
        
        # Calculate new columns
        grouped = self.state.groupby('County')
        self.state['Total Cases'] = grouped['New Cases'].cumsum()
        '''
